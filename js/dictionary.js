...
const logics = {
    "Логический оператор ИЛИ (||)": "Бинарный оператор<br>\n" +
        "Синтаксис:<br>\n" +
    ...
    "Логический оператор И (&&)": "Бинарный оператор<br>\n" +
        "Синтаксис:<br>\n" +
        "<code>result=a && b</code><br>\n" +
        "При работе с булевыми операндами:<br>\n" +
        "- если какой-либо из операндов вернет false, оператор И вернет false<br>\n" +
        "- только если оба операнда true, оператор И вернет true<br>\n" +
        "Если операнд не логического типа, будет приведен к логическому типу<br>\n" +
        "В JavaScript оператор И работает с несколькими значениями:<br>\n" +
        "<code>result=value1 && value2 && value3</code><br>\n" +
        " -\tВычисляет операнды слева направо<br>\n" +
        " -\tКаждый операнд конвертируется в логическое значение для вычисления (возвращается исходное значение операнда)<br>\n" +
        " -\tЦепочка И возвращает первое ложное значение, а дальнейшее вычисление останавливается ('сокращенное вычисление')<br>\n" +
        " -\tЕсли все операнды true, возвращается последний из них<br>\n" +
        " <strong>Приоритет оператора И && больше, чем ИЛИ ||</strong>\n" +
        "\n",

    "Логический оператор НЕ (!)": "Унарный оператор<br>\n" +
        "Синтаксис:<br>\n" +
        "<code>result = !value</code><br>\n" +
        ...
        "- Затем возвращает противоположное значение.<br>\n" +
        "<code>alert (!true); // false</code><br>\n" +
        "<b>Двойное НЕ !! </b>используют для преобразования значений к логическому типу:<br>\n" +
        "<code>alert (!!null); // false</code><br>\n" +
        "<strong>Приоритет НЕ ! является наивысшим из всех логических операторов, поэтому он всегда выполняется первым, перед && или ||.</strong>\n",

    "Оператор нулевого слияния (??)": "Бинарный оператор<br>\n" +
        "Был добавлен в язык недавно. В старых браузерах может понадобиться полифил<br>\n" +
        "Синтаксис:<br>\n" +
        "<code>result = a ?? b</code><br>\n" +
        "Оператор ?? возвращает первый аргумент, если он не null/undefined, иначе возвращает второй.<br>\n" +
        "<b>Различие между ?? и ||:</b><br>\n" +
        "-\t|| возвращает первое истинное значение.<br>\n" +
        "-\t?? возвращает первое значение, которое не является null/undefined.<br>\n" +
        "Оператор ?? имеет очень низкий приоритет, лишь немного выше, чем у ? и =, поэтому при использовании его в выражении, скорее всего, потребуются скобки.<br>\n" +
        "<strong>Запрещено использовать вместе с || или && без явно указанного приоритета, то есть без скобок.</strong>\n"
};
